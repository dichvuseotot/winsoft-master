    ...
    <>
        ...
        <Grid3>
          ...
          <FieldGroup label="Giới tính" required>
            <select style={inputStyle} value={form.gioiTinh} onChange={set("gioiTinh")}>
          ...
          <FieldGroup label="Đối tượng khám" required>
            <select style={inputStyle} value={form.doiTuongKham} onChange={set("doiTuongKham")}>
          ...
          <FieldGroup label="Số nhà/thôn/xóm" required>
            ...
            <select style={inputStyle} value={form.ngheNghiep} onChange={set("ngheNghiep")}>
...
function Tab2Content({ form, setForm }) {
  ...
  return (
    <Card>
      ...
      />
      <Grid3>
        <FieldGroup label="Mã CSKCB" helper="Giá trị được hệ thống quản lý theo CSYT.">
          <input style={inputStyle} type="text" value={form.maCskcb} onChange={set("maCskcb")} />
        </FieldGroup>
        <FieldGroup label="Mã CSKCB theo chuẩn GLN" required>
          <input style={inputStyle} type="text" value={form.maCskcbGln} onChange={set("maCskcbGln")} />
        </FieldGroup>
        <FieldGroup label="Ngày khám sức khỏe" required>
          <input style={inputStyle} type="date" value={form.ngayKham} onChange={set("ngayKham")} />
        </FieldGroup>
        <FieldGroup label="Giờ khám">
          <input style={inputStyle} type="time" value={form.gioKham} onChange={set("gioKham")} />
        </FieldGroup>
        <div /><div />
      </Grid3>
    </Card>
      ...
      {["Không", "Có"].map((opt) => (
        <label
          ...
          style={{
            ...
            color: value === opt ? "var(--primary)" : "var(--gray-700, #374151)",
          }}
        >
          <input
            type="radio"
    ...
    <div style={{ border: "1px solid var(--gray-200)", borderRadius: 8, overflow: "hidden" }}>
      ...
      {items.map((item, i) => {
        ...
        return (
          ...
          >
            <span style={{ fontSize: ".78rem", color: "var(--gray-400)" }}>{startIdx + i + 1}</span>
            <span style={{ fontSize: ".83rem", color: "var(--gray-800)", lineHeight: 1.4 }}>{item}</span>
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
              <RadioKhongCo name={key} value={form[key] || "Không"} onChange={setRadio(key)} />
            </div>
          </div>
        );
      })}
  ...
  return (
    <>
        ...
        <div style={{
          ...
          <p style={{ fontSize: ".84rem", color: "var(--gray-700)", marginBottom: ".75rem", lineHeight: 1.5 }}>
            Có ai trong gia đình mắc một trong các bệnh: truyền nhiễm, tim mạch, đái tháo đường,
            lao, hen phế quản, ung thư, động kinh, rối loạn tâm thần không?
          </p>
          <div style={{ display: "flex", gap: "1.5rem" }}>
            {[["Không", "a, Không"], ["Có", "b, Có"]].map(([val, lbl]) => (
              <label key={val} style={{
                display: "flex", alignItems: "center", gap: ".35rem",
                cursor: "pointer", fontSize: ".85rem",
                fontWeight: (form.gdChung || "Không") === val ? 600 : 400,
                color: (form.gdChung || "Không") === val ? "var(--primary)" : "var(--gray-700)",
              }}>
                <input
                  type="radio"
                  name="gdChung"
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
          <BanThanTable items={left}  startIdx={0}  form={form} setRadio={setRadio} />
          <BanThanTable items={right} startIdx={11} form={form} setRadio={setRadio} />
        </div>
      </Card>
...
function Tab4Content({ form, setForm }) {
  ...
  const bmi =
    form.chieuCao && form.canNang
      ? (parseFloat(form.canNang) / Math.pow(parseFloat(form.chieuCao) / 100, 2)).toFixed(1)
      : "";
    ...
    <Card>
      ...
      <Grid6>
        <FieldGroup label="Chiều cao (cm)">
          <input style={inputStyle} type="number" min="50" max="250"
            value={form.chieuCao || ""} onChange={set("chieuCao")} />
        </FieldGroup>

        <FieldGroup label="Cân nặng (kg)">
          <input style={inputStyle} type="number" min="1" max="300"
            value={form.canNang || ""} onChange={set("canNang")} />
        </FieldGroup>

        <FieldGroup label="Chỉ số BMI">
          <input style={{ ...inputStyle, background: "var(--gray-50)" }}
            type="text" readOnly value={bmi} />
        </FieldGroup>

        <FieldGroup label="Mạch (lần/phút)">
          <input style={inputStyle} type="number" min="30" max="250"
            value={form.mach || ""} onChange={set("mach")} />
        </FieldGroup>

        <FieldGroup label="Huyết áp (mmHg)">
          <input style={inputStyle} type="text" placeholder="VD: 120/80"
            value={form.huyetAp || ""} onChange={set("huyetAp")} />
        </FieldGroup>

        <FieldGroup label="Phân loại thể lực">
          <select style={inputStyle} value={form.phanLoaiTheLuc || "Loại II (Khỏe)"} onChange={set("phanLoaiTheLuc")}>
...
function SignatureBox({ id }) {
  ...
  const onDown = (e) => { drawing.current = true; lastPos.current = getXY(e); };
  const onMove = (e) => {
  ...
  return (
    <div style={{ border: "1px solid var(--gray-200)", borderRadius: 8, overflow: "hidden", background: "var(--gray-100)" }}>
            <div style={{ display: "flex" }}>
        {[["sign", "✍️ Ký tên", "#2563eb"], ["upload", "📁 Upload", "#d97706"]].map(([v, lbl, col]) => (
          <button key={v} onClick={() => setTab(v)} style={{
            flex: 1, padding: ".35rem .5rem", border: "none", background: "none",
        ...
        <>
          ...
          />
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: ".25rem .6rem", background: "var(--gray-50)", borderTop: "1px solid var(--gray-100)" }}>
            <button onClick={clear} style={{ border: "none", background: "none", cursor: "pointer", fontSize: ".72rem", color: "var(--gray-700)", padding: 0 }}>Xóa nét</button>
            <div style={{ display: "flex", alignItems: "center", gap: ".5rem" }}>
              <span style={{ fontSize: ".72rem", color: "var(--gray-400)" }}>Bấm ra ngoài để hoàn thành</span>
              {hasSig && (
                <button style={{
                  height: 26, padding: "0 .6rem",
        ...
        <div style={{ padding: ".75rem 1rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: ".75rem" }}>
            <label htmlFor={`sig_${id}`} style={{
              display: "inline-flex", alignItems: "center",
              padding: ".35rem .9rem", background: "#1f2937", color: "#fff",
              borderRadius: 6, cursor: "pointer", fontSize: ".82rem", fontWeight: 600,
              whiteSpace: "nowrap", flexShrink: 0,
            }}>
              Chọn tệp
              <input id={`sig_${id}`} type="file" accept="image/*" style={{ display: "none" }}
          ...
          <div style={{ fontSize: ".72rem", color: "#2563eb", marginTop: ".4rem" }}>
...
const PHAN_LOAI_LS = ["Loại I (Tốt)", "Loại II (Khỏe)", "Loại III (Trung bình)", "Loại IV (Yếu)", "Loại V (Kém)"];
const THI_LUC_OPT = ["10/10","9/10","8/10","7/10","6/10","5/10","4/10","3/10","2/10","1/10","< 1/10"];

function Tab5Content({ form, setForm }) {
  const set = (key) => (e) => setForm((p) => ({ ...p, [key]: e.target.value }));

  const plSelect = (key) => (
    <select style={inputStyle} value={form[key] || "Loại II (Khỏe)"} onChange={set(key)}>
      {PHAN_LOAI_LS.map((o) => <option key={o}>{o}</option>)}
    </select>
  );
  ...
  const tlSelect = (key) => (
    <select style={inputStyle} value={form[key] || ""} onChange={set(key)}>
      <option value="">—</option>
      {THI_LUC_OPT.map((o) => <option key={o}>{o}</option>)}
    </select>
  );

  const NK = ({ label, fk }) => (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "1.25rem", padding: "1rem 0", borderBottom: "1px solid var(--gray-100)", alignItems: "start" }}>
      <div>
        <div style={{ fontSize: ".84rem", color: "var(--gray-700)", fontWeight: 500, marginBottom: ".5rem" }}>{label}</div>
        <input style={{ ...inputStyle, marginBottom: ".5rem" }} type="text" value={form[fk] || ""} onChange={set(fk)} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "1.25rem", alignItems: "start" }}>
        <div>
          <FieldGroup label={resultLabel}>
            <input style={inputStyle} type="text" value={form[fk] || ""} onChange={set(fk)} />
          </FieldGroup>
  ...
  return (
    <Card>
      <SectionHeader num="5" title="5. Khám lâm sàng" subtitle="Nội khoa, Ngoại khoa, Da liễu, Sản phụ khoa, Mắt, TMH, RHM" />

            <div style={subLabelStyle}>1. Khám nội khoa</div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "1.25rem", marginBottom: ".25rem" }}>
        <div />
        <div style={{ fontSize: ".75rem", fontWeight: 600, color: "var(--gray-700)", textAlign: "center" }}>Họ tên và chữ ký của bác sĩ</div>
      </div>
      ...
      <NK label="h, Tâm thần"                   fk="nk_h" />
            ...
            <SimpleExam secLabel="2. Ngoại khoa" resultLabel="Kết quả khám ngoại khoa" fk="ngk" />

            <SimpleExam secLabel="3. Da liễu" resultLabel="Kết quả khám da liễu" fk="dl" />

            <SimpleExam secLabel="4. Sản phụ khoa" resultLabel="Kết quả khám sản phụ khoa" fk="spk" />

            <div style={{ marginTop: "1.25rem" }}>
        <div style={subLabelStyle}>5. Khám mắt</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "1.25rem", alignItems: "start" }}>
          <div>
            <div style={{ fontSize: ".82rem", fontWeight: 600, color: "var(--gray-700)", marginBottom: ".6rem" }}>Kết quả khám thị lực:</div>
            <div style={{ display: "grid", gridTemplateColumns: "auto 1fr 1fr", gap: ".5rem 1rem", alignItems: "center", marginBottom: ".5rem" }}>
              <span />
              <label style={{ ...labelStyle, marginBottom: 0, textAlign: "center" }}>Trái</label>
              <label style={{ ...labelStyle, marginBottom: 0, textAlign: "center" }}>Phải</label>
              <label style={{ ...labelStyle, marginBottom: 0, whiteSpace: "nowrap" }}>Thị lực không kính:</label>
              {tlSelect("mat_kk_trai")}
              {tlSelect("mat_kk_phai")}
              <label style={{ ...labelStyle, marginBottom: 0, whiteSpace: "nowrap" }}>Thị lực có kính:</label>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".75rem", marginTop: ".25rem" }}>
              <FieldGroup label="Các bệnh về mắt (nếu có)">
                <input style={inputStyle} type="text" value={form.mat_benh || ""} onChange={set("mat_benh")} />
              </FieldGroup>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "1.25rem", alignItems: "start" }}>
          <div>
            <div style={{ fontSize: ".82rem", fontWeight: 600, color: "var(--gray-700)", marginBottom: ".6rem" }}>Kết quả khám thính lực:</div>
            <div style={{ display: "grid", gridTemplateColumns: "auto 1fr 1fr", gap: ".5rem 1rem", alignItems: "center", marginBottom: ".5rem" }}>
              <span />
              <label style={{ ...labelStyle, marginBottom: 0, textAlign: "center" }}>Tai trái</label>
              <label style={{ ...labelStyle, marginBottom: 0, textAlign: "center" }}>Tai phải</label>
              <label style={{ ...labelStyle, marginBottom: 0, whiteSpace: "nowrap" }}>Thính lực nói thường (m):</label>
              <input style={inputStyle} type="number" value={form.tmh_tt_trai || ""} onChange={set("tmh_tt_trai")} />
              <input style={inputStyle} type="number" value={form.tmh_tt_phai || ""} onChange={set("tmh_tt_phai")} />
              <label style={{ ...labelStyle, marginBottom: 0, whiteSpace: "nowrap" }}>Thính lực nói thầm (m):</label>
              <input style={inputStyle} type="number" value={form.tmh_th_trai || ""} onChange={set("tmh_th_trai")} />
              <input style={inputStyle} type="number" value={form.tmh_th_phai || ""} onChange={set("tmh_th_phai")} />
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".75rem", marginTop: ".25rem" }}>
              <FieldGroup label="Các bệnh về tai mũi họng (nếu có)">
                <input style={inputStyle} type="text" value={form.tmh_benh || ""} onChange={set("tmh_benh")} />
              </FieldGroup>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "1.25rem", alignItems: "start" }}>
          <div>
            <FieldGroup label="Kết quả khám răng - hàm - mặt:">
              <input style={inputStyle} type="text" value={form.rhm_ketQua || ""} onChange={set("rhm_ketQua")} />
            </FieldGroup>
            <div style={{ display: "grid", gridTemplateColumns: "auto 1fr 1fr", gap: ".5rem 1rem", alignItems: "center", margin: ".5rem 0" }}>
              <label style={{ ...labelStyle, marginBottom: 0 }}>Kết quả khám hàm:</label>
              <FieldGroup label="Hàm trên">
                <input style={inputStyle} type="text" value={form.rhm_hamTren || ""} onChange={set("rhm_hamTren")} />
              </FieldGroup>
              <FieldGroup label="Hàm dưới">
                <input style={inputStyle} type="text" value={form.rhm_hamDuoi || ""} onChange={set("rhm_hamDuoi")} />
              </FieldGroup>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".75rem" }}>
              <FieldGroup label="Các bệnh về răng-hàm-mặt (nếu có)">
                <input style={inputStyle} type="text" value={form.rhm_benh || ""} onChange={set("rhm_benh")} />
              </FieldGroup>
...
function Tab6Content({ form, setForm }) {
  ...
  const fieldLabel = (text) => (
    <div style={{ fontSize: ".82rem", color: "var(--gray-700)", marginBottom: ".35rem" }}>{text}</div>
  );

  return (
    <Card>
      <SectionHeader num="6" title="6. Cận lâm sàng"
      ...
      <div style={{ marginBottom: "1.25rem" }}>
        ...
        <input style={{ ...inputStyle }} type="text"
          ...
          value={form.cls_cdha || ""} onChange={set("cls_cdha")} />
      </div>

            {secLabel("D. Kết quả khám cận lâm sàng khác")}

      <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: "1.25rem", alignItems: "start", marginBottom: "1.25rem" }}>
        <div>
          <div style={{ fontSize: ".84rem", color: "var(--gray-700)", marginBottom: ".5rem" }}>
          ...
          <div style={{ display: "flex", gap: "1.25rem" }}>
            {["Không", "Có"].map((val) => (
              <label key={val} style={{
                ...
                color: (form.cls_coKhac || "Không") === val ? "var(--primary)" : "var(--gray-700)",
              }}>
                <input type="radio" name="cls_coKhac" value={val}
        ...
        <div>
          <div style={{ fontSize: ".75rem", fontWeight: 600, color: "var(--gray-700)", textAlign: "center", marginBottom: ".35rem" }}>
        ...
        paddingTop: "1rem", borderTop: "1px solid var(--gray-100)",
      }}>
        <span style={{ fontSize: ".84rem", fontWeight: 600, color: "#2563eb", whiteSpace: "nowrap" }}>
        ...
        >
          {CLS_CHISO_OPTIONS.map((o) => (
            <option key={o} value={o === "-- Chọn Chỉ Số --" ? "" : o}>{o}</option>
          ))}
        </select>
        <button style={{
          height: 38, padding: "0 1rem", whiteSpace: "nowrap",
          background: "#2563eb", color: "#fff", border: "none",
          borderRadius: "var(--radius-md, 8px)", fontWeight: 600,
          fontSize: ".84rem", cursor: "pointer", fontFamily: "var(--font-body)",
          flexShrink: 0,
        }}>
...
function Tab7Content({ form, setForm }) {
  ...
  return (
    <Card>
      <SectionHeader num="7" title="7. Kết luận & Chẩn đoán"
        subtitle="Phân loại sức khỏe tổng quát, danh mục ICD-10 và xác nhận chữ ký bác sĩ" />

      <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: "2rem", alignItems: "start" }}>
                <div>
                    <div style={{ ...subLabelStyle, marginBottom: ".75rem" }}>1. Tình trạng bệnh</div>

          <FieldGroup label="Chọn danh mục ICD-10">
            <select style={inputStyle} value={form.kl_icd10 || ""} onChange={set("kl_icd10")}>
              <option value="">Chọn hoặc tìm kiếm danh mục ICD-10...</option>
              {ICD10_OPTIONS.map((o) => <option key={o} value={o}>{o}</option>)}
            </select>
          </FieldGroup>
          ...
          <div style={{ marginTop: ".75rem" }}>
            ...
            />
                    ...
                    <div style={{ ...subLabelStyle, marginTop: "1.25rem", marginBottom: ".75rem" }}>2. Phân loại sức khỏe</div>

          <FieldGroup label="Chọn phân loại sức khỏe" required>
            <select style={inputStyle} value={form.kl_phanLoai || "Loại II - Khoẻ"} onChange={set("kl_phanLoai")}>
              {PHAN_LOAI_SK.map((o) => <option key={o} value={o}>{o}</option>)}
            </select>
          </FieldGroup>
        </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: ".4rem", marginBottom: ".25rem" }}>
              <span>✍️</span>
              <span style={{ fontSize: ".8rem", fontWeight: 700, color: "var(--gray-700)" }}>Chữ ký Bác sĩ kết luận</span>
            </div>
            <div style={{ fontSize: ".78rem", color: "var(--gray-700)", marginBottom: ".4rem" }}>Người kết luận</div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: ".4rem", marginBottom: ".25rem" }}>
              <span>🏛</span>
              <span style={{ fontSize: ".8rem", fontWeight: 700, color: "var(--gray-700)" }}>Chữ ký & con dấu Đại diện Cơ sở KCB</span>
            </div>
            <div style={{ fontSize: ".78rem", color: "var(--gray-700)", marginBottom: ".4rem" }}>Đại diện Cơ sở y tế (Bệnh viện)</div>
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: ".5rem", flexWrap: "wrap" }}>
              <span style={{
                ...
                fontSize: "1rem", color: "var(--gray-900, #111827)", whiteSpace: "nowrap",
              }}>
                Nhập tay hồ sơ khám sức khỏe
              </span>
              <span style={{
                background: "#eff6ff", color: "#2563eb", fontSize: ".72rem",
                fontWeight: 600, padding: ".2em .6em", borderRadius: 99,
                border: "1px solid #bfdbfe", whiteSpace: "nowrap",
              }}>
                KSK_TREN_18 (Trên 18) · v1
              </span>
            </div>
            <div style={{ fontSize: ".78rem", color: "var(--gray-500, #6b7280)", marginTop: ".15rem" }}>
              CSYT: 75407 — Vui lòng chọn
            </div>
          </div>

                    <div style={{ display: "flex", alignItems: "center", gap: ".625rem", flexShrink: 0, flexWrap: "wrap" }}>
            <label style={{ fontSize: ".78rem", fontWeight: 600, color: "var(--gray-700, #374151)", whiteSpace: "nowrap" }}>
              Chọn mẫu:
            </label>
            <select
              value={mauPhieu}
              ...
              style={{
                height: "36px", border: "1.5px solid var(--gray-200, #e5e7eb)",
                borderRadius: "var(--radius-md, 8px)", padding: "0 .625rem",
                fontSize: ".8rem", outline: "none", maxWidth: "260px",
                fontFamily: "var(--font-body)", background: "var(--gray-100)",
            ...
            <button style={{
              height: "36px", padding: "0 1.1rem", background: "#16a34a",
              color: "#fff", border: "none", borderRadius: "var(--radius-md, 8px)",
            ...
            <button
              onClick={() => navigate("/ho-so-ksk")}
              style={{
                height: "36px", padding: "0 1.1rem", background: "var(--gray-100)",
                color: "var(--gray-700, #374151)", border: "1.5px solid var(--gray-300, #d1d5db)",
                borderRadius: "var(--radius-md, 8px)", fontWeight: 600,
                fontSize: ".85rem", cursor: "pointer", fontFamily: "var(--font-body)", whiteSpace: "nowrap",
              }}
            >
              Quay lại
            </button>
            ...
            return (
              ...
              >
                ...
                <span style={{
                  ...
                  padding: ".1em .45em", borderRadius: 99,
